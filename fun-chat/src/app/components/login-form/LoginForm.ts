...
export default class LoginForm extends BaseComponent {
  ...
  private setForm() {
    ...
    this.nameInput = input([styles.input, 'name-input'], 'name', 'name', 'Enter your Name', 'true');
    this.surnameInput = input([styles.input], 'surname', 'surname', 'Enter your Surname', 'true');
    ...
    const surnameTextField = div([styles['text-field']], surnameLabel, this.surnameInput, surnameErrorMessageElement);
  ...
  static validateForm(inputElement: HTMLInputElement, errorMessageSelector: string, length: number) {
    ...
    if (inputValue.length < length) {
      const errorMessage = `Minimum length of the ${length === 3 ? 'first name' : 'surname'} is ${length} characters`;
